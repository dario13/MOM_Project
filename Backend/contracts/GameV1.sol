...
import {DepositAlreadyMade, DepositNotMade, InsufficientTokensInReserve, IncorrectTokenAllowance, NoMatchCreated} from './Errors.sol';
...
contract GameV1 is Ownership {
    ...
    struct DepositRegister {
        bool isDepositMade;
        Difficulty difficulty;
    }
    mapping(address => DepositRegister) deposits;
    ...
    function getRules(Difficulty _difficulty)
        public
        pure
        returns (
            uint8 tokensToPlay,
    ...
    function depositTokens(Difficulty _difficulty) public {
        if (deposits[msg.sender].isDepositMade == true) revert DepositAlreadyMade();
        uint256 allowance = token.allowance(msg.sender, address(this));
        (uint8 tokensToPlay, , ) = getRules(_difficulty);
        if (allowance != tokensToPlay) revert IncorrectTokenAllowance();
        deposits[msg.sender] = DepositRegister(true, _difficulty);
        token.safeTransferFrom(msg.sender, address(this), tokensToPlay);
    }

    function createMatch() external {
        if (deposits[msg.sender].isDepositMade == false) revert DepositNotMade();
        deposits[msg.sender].isDepositMade = false;
        Difficulty _difficulty = deposits[msg.sender].difficulty;
        (, , uint8 cardsToWin) = getRules(_difficulty);
